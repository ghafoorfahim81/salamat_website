...
@section('title')
    د خدماتو ریاست - لیست اسناد
@endsection
@section('content')
    <div id="app">
        <!-- Breadcrumb -->
        <d-header :title="`{{__('setting.document_type_list')}}`" :show-icon="false"></d-header>
        ...
        <div class="w-full bg-white rounded-lg shadow mt-1 dark:bg-gray-800">
            <div class="w-full mx-auto max-w-screen-xl p-4 md:flex md:items-center md:justify-between">
                ...
                @if(hasPermission(['document_type_create']))
                    <div
                        class="flex flex-wrap items-center mt-3 text-sm font-medium text-gray-500 dark:text-gray-400 sm:mt-0">
                        <add-new @click="openModal" :btn_name="`{{__('setting.add_document_type')}}`"/>
                    </div>
                @endif
            </div>
            <div class="mt-2">
                <datatable ref="child" :per-page="{{perPage()}}"
                    ...
                    <template slot="tbody">
                        ...
                        <tr v-for="(record,index) in apiData.data" :key="record.id"
                            ...
                            <td scope="row"
                                class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                <action-btn @edit="editRecord(record.id)"
            ...
            @include('document_types.create-edit-modal')
            ...
            data() {
                return {
                    ...
                    columns: [
                        ...
                        {
                            label: "@lang('setting.document_type')",
                            sort: true,
                            name: 'name',
                        },
                    ...
                    permissions: {
                        delete: "{!! hasPermission(['document_type_delete']) !!}",
                        edit: "{!! hasPermission(['document_type_edit']) !!}",
                        view: "{!! hasPermission(['document_type_view']) !!}",
                    },
            ...
            methods: {
                ...
                saveRecord() {
                    if (this.form.name && this.form.name !== ' ') {
                        this.save_types_loader = true;
                        axios.post("{{route('document-types.store')}}", this.form).then((res) => {
                            let response = res.data;

                            if (response.status == 200) {
                                this.save_types_loader = false;
                                this.isModalOpen = false;
                ...
                editRecord(id = null, index) {
                    ...
                    this.id = id;
                    this.isModalOpen = true;
                    axios.get(`{!!url('document-types/edit')!!}/${id}`).then((res) => {
                ...
                updateRecord() {
                    if (this.form.name && this.form.name !== ' ') {
                        this.save_types_loader = true;
                        axios.patch(`{!!url('document-types/update')!!}/${this.id}`, this.form).then((res) => {
                            let response = res.data;
